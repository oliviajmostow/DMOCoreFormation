...
def movie_maker_call():
    snapshot_dir='/home/cchayward/scratch/VoT/sbcn_hr_f'
    output_dir='/home/cchayward/scratch/VoT/sbcn_hr_f/movie_frames'
    cosmo=0.
    size=20.
    type_to_make='star'
    movie_maker(xmax_of_box=size,snapdir=snapshot_dir,outputdir_master=output_dir,\
        show_gasstarxray=type_to_make,cosmological=cosmo,frame_min=0,frames_per_gyr=50.,\
        i_snap_min=57,i_snap_max=72,skip_bh=1,set_fixed_center=[0.,0.,0.],\
        theta_0=90.,phi_0=180.,add_extension='',\
        camera_opening_angle=80.,\
        pixels=500);
    # i_snap_min=53; i_snap_max=66;frames_per_gyr=400.
...
def build_camera_centering( snums, snapdir , 
    ...
    for i in range(n_snums):
        ...
        if (center_on_com==1):
            ...
            if (PPP['k']==1):
                if (PPP['m'].size > 0):
                    wt = np.array(P['m']); wt /= np.sum(wt); p = np.array(P['p'])
